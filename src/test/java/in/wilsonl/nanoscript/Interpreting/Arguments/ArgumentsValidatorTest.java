...
public class ArgumentsValidatorTest {
    private void test(Builder config) {
        ...
        for (Map.Entry<String, NSData> expectedArg : config.expected.entrySet()) {
            assertTrue(expectedArg.getValue().nsTestEquality(result.get(expectedArg.getKey())).isTrue());
        }
        if (varLenResult != null) {
            for (int i = 0; i < config.expectedVarLenArgs.get().size(); i++) {
                assertTrue(i < varLenResult.size());
                assertTrue(config.expectedVarLenArgs.get().get(i).nsTestEquality(varLenResult.get(i)).isTrue());
            }
        }
    }
    ...
    private static class Builder {
        ...
        public Builder addExpectedVarLenArg(NSData value) {
            expectedVarLenArgs.get().add(value);
            return this;
        }
    }
