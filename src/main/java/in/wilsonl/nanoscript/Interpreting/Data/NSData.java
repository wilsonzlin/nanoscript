...
public abstract class NSData<T> {
    ...
    public NSIterator iterate() {
        throw new UnsupportedOperationError(String.format("%s cannot be iterated", type));
    }

    public NSData<?> applyUnaryOperator(Operator operator) {
        throw new UnsupportedOperationError(String.format("The operator %s cannot be applied to %s", operator, type));
    }
    ...
    public NSData<?> applyCall(List<NSData<?>> arguments) {
        throw new UnsupportedOperationError(String.format("%s cannot be called", type));
    }

    public NSData<?> applyLookup(List<NSData<?>> terms) {
        throw new UnsupportedOperationError(String.format("%s cannot be looked up", type));
    }

    public NSData<?> applyUpdate(List<NSData<?>> terms, NSData<?> value) {
        throw new UnsupportedOperationError(String.format("%s cannot be updated", type));
    }

    public NSData<?> applyAccess(String member) {
        throw new UnsupportedOperationError(String.format("%s does not have any members", type));
    }
    ...
    public NSBoolean toNSBoolean() {
        throw new UnsupportedOperationError(String.format("%s cannot be converted to a boolean", type));
    }

    public NSString toNSString() {
        throw new UnsupportedOperationError(String.format("%s cannot be stringified", type));
    }
