...
public class ArgumentsValidator {
  ...
  private int matchArguments (int fakeFromInc, int fakeToInc, int realArgBoundary, List<NSArgument> args, boolean required) {
    ...
    int realParamFrom = direction == -1 ?
      parameters.length + fakeFromInc :
      fakeFromInc;
    int realParamTo = direction == -1 ?
      parameters.length + fakeToInc :
      fakeToInc;
    int realArgsFrom = direction == -1 ?
      argsCount + fakeFromInc :
      fakeFromInc;
    int realArgsTo = direction == -1 ?
      argsCount + fakeToInc :
      fakeToInc;
  ...
  public NSValidatedArguments match (List<NSArgument> args) {
     ...
     */
    int upperBoundFromLeft = (hasOptional ?
      posOfFirstOptional :
      posOfVarLen) - 1;
     ...
     */
    int lowerBoundFromRight = (hasOptional ?
      posOfLastOptional :
      posOfVarLen) - parameters.length + 1;
    int lastArgIdxFromRight = matchArguments(-1, lowerBoundFromRight, lastArgIdxFromLeft, args, true);

    if (hasOptional) {
      int optionalUpperBoundFromLeft = hasVarLen ?
        posOfVarLen - 1 :
        posOfLastOptional;
      ...
      if (hasVarLen) {
        ...
        lastArgIdxFromRight = matchArguments(
          posOfLastOptional - parameters.length,
          posOfVarLen - parameters.length + 1,
          lastArgIdxFromLeft,
          args,
          false);
